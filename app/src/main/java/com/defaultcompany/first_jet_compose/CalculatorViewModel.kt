...
class CalculatorViewModel:ViewModel() {
    ...
    fun onAction(action:CalculatorAction){
        when(action){
           ...
           is CalculatorAction.Operation -> enterOperations(action.operation)
    ...
    private fun performCalculation() {
        ...
        if(number1!=null && number2!=null){
            val result=when(state.operation){
                ...
                is CalculateOperation.Substract -> number1-number2
    ...
    private fun enterOperations(operation: CalculateOperation) {
    ...
    private fun enterNumber(number: Int) {
        if(state.operation==null){
            ...
            state=state.copy(number2 = state.number1+number)
            return
        }
